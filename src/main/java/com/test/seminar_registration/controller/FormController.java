...
public class FormController {
	...
	public String addForms(
		...
		@RequestParam String nomor_telepon,
		@RequestParam String id_line,
		@RequestParam String jurusan,
		@RequestParam String fakultas,
		@RequestParam String angkatan,
		Model model,
		SessionStatus sessionStatus
	) {if (formService.formRegisterChecker(email, nomor_telepon, id_line)) {
        model.addAttribute("error", "Form with this Email, Phone Number, or Line Id have already existed.");
        return "register.html";
    } else {
		Form f = new Form();
		f.setNama(nama);
		f.setNim(nim);
		f.setEmail(email);
		f.setNomorTelepon(nomor_telepon);
		f.setIdLine(id_line);
